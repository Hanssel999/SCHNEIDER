...
export function HosChart({ events, editable = false, onUpdate }: HosChartProps) {
    ...
    console.log("orderedEvents", events);
    ...
    const workEvents = orderedEvents.filter((event) => normalizedStatus(event.status) === "driving" || normalizedStatus(event.status) === "on duty");
    ...
    const notDrivingSections = orderedEvents.filter((event) => nonDrivingStatuses.has(normalizedStatus(event.status)) && toMinutes(event.start) >= workStart && toMinutes(event.end) <= workEnd && event.end !== "24:00");
    ...
    return (
        <div className="hos-chart-wrap">
            <svg aria-label="Hours of service line chart with 15 minute intervals" className="hos-chart" role="img" viewBox={`0 0 ${width} ${height}`}>
                ...
                {notDrivingSections.map((event) => { const sectionStart = x(toMinutes(event.start)); const sectionEnd = x(toMinutes(event.end)); const sectionMid = (sectionStart + sectionEnd) / 2; return <g className="chart-not-driving-section" key={`not-driving-${event.id}`}><path d={`M${sectionStart},${height - 141} L${sectionStart},${height - 120} L${sectionEnd},${height - 120} L${sectionEnd},${height - 141}`} /><text textAnchor="middle" x={sectionMid} y={height - 103}>{event.status.toUpperCase()}</text><text className="chart-section-info" textAnchor="middle" x={sectionMid} y={height - 85}>{event.note}</text></g>; })}
